...
*(切换账号后工具会尝试自动刷新 Codex，使账号立即生效；检测到 Pencil MCP 时会自动加兼容代理，并关闭 PowerShell 的 shell_snapshot；Windows 下优先自动重启 Codex 桌面端以避免错误页)*
...
*(After switching accounts, the tool will attempt to refresh Codex so changes take effect immediately; if the Pencil MCP server is detected, a compatibility proxy is added and PowerShell shell_snapshot is disabled; on Windows it will prefer an automatic Codex desktop restart to avoid error pages.)*
